...
class Population(object):
    ...
    def random_seq(self, img_target, n_circles, i):
        ...
        # seq[0] = cur_row
        # seq[1] = cur_col
        # seq[2] = 0.25 # initial radius
        seq = self.lbs + seq.T*(self.ubs - self.lbs)
        # seq[0][3] = img_target.mean()-0.1 + 0.2*np.random.rand()
        return seq[0]
...
def update_population(P, img_target, args):
    old_cells = P.cells
    fitnesses = [cell.fitness for cell in old_cells]
    old_cells = sorted(old_cells, key=lambda cell: cell.fitness, reverse=True)
...
def train(args):
    ...
    # init random circle population
    # img_size = img_target.shape
    # lbs = np.array([0.0, 0.0, 0.0, 0.0])
    # ubs = np.array([img_size[0], img_size[1], np.min(img_size)/10., 1.0])
    ...
    for i in range(args.num_epochs):
        ...
        if i % 10 == 0:
            ...
            print('{} cells with best fitness {}, median fitness {}, and minimum fitness {}'.format(len(P.cells), best_cell.fitness, np.median(fs), fs.min()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pct_reproduce', type=float, default=0.1)
    parser.add_argument('--prob_feature_mutate', type=float, default=0.5)
    parser.add_argument('--prob_shuffle', type=float, default=0.2)
    parser.add_argument('--noise_var', type=float, default=0.0003) # 0.0001
